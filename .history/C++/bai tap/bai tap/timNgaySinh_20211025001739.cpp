...
int main(){
    ...
    if(month==1 || month ==3 || month == 5 || month ==7 || month ==8 || month ==10){
        if(s > 31) cout<<"01/"<<"0"<<month+1<<"/"<<year;
        //printf("%02d/%02d/%02d",s,month,year);
        else printf("%02d/%02d/%02d",s,month,year);
    }
    if(month==12){
        if(s>31) cout<<"01/"<<"01/"<<year+1;
        else cout<<s<<"/"<<"0"<<month<<"/" <<year;
    }
    if(month==4 || month ==6 || month == 9 || month ==11){
        if(s > 30) cout<<"01/"<<"0"<<month+1<<"/"<<year;
        else cout<<s<<"/"<<"0"<<month<< "/" <<year;
    }
    if(month==2){
        if(year %4 ==0 && year !=0){
            if(s > 29) cout<<"01/"<<"0"<<month+1<<"/"<<year;
            else cout<<s<<"/"<<month<< "/" <<year;
        }  
        else{
            if(s > 28) cout<<"01/"<<"0"<<month+1<<"/"<<year;
            else cout<<s<<"/"<<"0"<<month<< "/" <<year;
        }
    }
    }
