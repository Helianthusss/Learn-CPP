...
class BenhNhan{
    private:
        string code; //mã b?nh nhân
        string name;
        int day; //s? ngày cách ly
        string infection; //lây nhi?m t? ai
    public:
        BenhNhan(){
            this->code = "12345";
            this->name = "Meo";
        ...
        friend istream& operator>>(istream &input, BenhNhan &benhnhan){
            ...
            do{
                cout<<"Infection: ";
                fflush(stdin);
                cin >> benhnhan.infection;
            }while((benhnhan.infection != "NO") && (benhnhan.infection.size() != 5));
        }
        void display(){
            cout<<"Code: "<<code<<endl;
            cout<<"Name: "<<name<<endl;
